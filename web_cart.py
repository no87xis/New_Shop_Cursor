"""
Веб-страница корзины с исправленными сессиями
"""
...
async def cart_page(request: Request):
    """
    Страница корзины с автоматическим созданием сессии
    """
    try:
...
<body class="bg-gray-100 min-h-screen">
    ...
    <!-- JavaScript -->
    <script>
        console.log('Cart page loaded, starting loadCart...');
        console.log('Session ID: {session_id}');
        
        async function loadCart() {{
            console.log('loadCart function called');
            try {{
                const response = await fetch("/api/shop/cart/items", { credentials: "include" });
                ...
                if (data.items && data.items.length > 0) {{
                    console.log('Items found, rendering cart items');
